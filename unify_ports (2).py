...
def replace_in_file(path: Path):
    ...
    # 1. Python files – smash any port numbers
    if path.suffix.lower() == ".py":
        # Hardcoded ports → env vars
        content = re.sub(
            r"(\bport\s*[:=])\s*[\'\"]?(\d+)[\'\"]?",
        ...
        content = re.sub(r"port= os.getenv('HTTP_PORT' if 3001 in ('3000','3000','3000') else 'WS_PORT', '3001')|port= os.getenv('HTTP_PORT' if 3001 in ('3000','3000','3000') else 'WS_PORT', '3001')|port= os.getenv('HTTP_PORT' if 3001 in ('3000','3000','3000') else 'WS_PORT', '3001')|port= os.getenv('HTTP_PORT' if 3000 in ('3000','3000','3000') else 'WS_PORT', '3000'), "port=int(os.getenv('WS_PORT', 3001))", content)
    ...
    # 3. docker-compose.yml – force ports mapping + env
...
"""
    ...
    if content != original:
        ...
        logging.info(f"Unified → {path}")

def main():
    logging.info("CHIMERA PORT UNIFIER – FORCING 3000/3001 EVERYWHERE")
    ...
    logging.info("DONE. All ports smashed to HTTP_PORT=3000 and WS_PORT=3001. Total consistency achieved.")
