...
try:
    from tower_integration import TowerAdapter, TowerAPIKey, TowerCredentials
...
class DroxAITowerAdapter(TowerAdapter):
    ...
    def __init__(
        ...
        tower_url = tower_url or os.getenv(
            "DROXAI_TOWER_URL", "http://localhost:8001")
        ...
        self.role_mapping = {
            # DroxAI Tower roles → CHIMERA roles
            "admin": Role.ADMIN,
            "master": Role.ADMIN,
    ...
    def map_tower_scopes_to_capabilities(self, scopes: Set[str]) -> Set[str]:
        ...
        - "models:codellama" → Code completion access
        - "models:chat" → Chat access
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            async with httpx.AsyncClient(timeout=5.0) as client:
                ...
                # Check Enterprise AI (port 8000)
                enterprise_ai_url = self.tower_url.replace(":8001", ":8000")
                try:
...
async def example_droxai_integration():
    ...
    if not CHIMERA_AVAILABLE:
        print("❌ CHIMERA security module not available")
        return
    ...
    print("🔗 Connecting to DroxAI Tower...")
    droxai = await integrate_droxai_tower_with_chimera(security)
    ...
    print(f"   DroxAI Tower: {health['droxai_tower']['status']}")
    print(f"   Enterprise AI: {health['enterprise_ai']['status']}")
...
if __name__ == "__main__":
    ...
    2. Enterprise AI running on port 8000
    ...
    if not CHIMERA_AVAILABLE:
        print("❌ CHIMERA modules not available")
        sys.exit(1)

    print("="*60)
    print("CHIMERA ↔ DroxAI Tower Integration Test")
    print("="*60)
    ...
    print("="*60)
    print("\n📝 Next Steps:")
    print("1. Start DroxAI Tower: python droxai_tower.py --port 8001")
    print("2. Start Enterprise AI: python enterprise_ai.py")
    print("3. Start CHIMERA: python chimera_autarch.py")
    print("4. Use DroxAI keys to authenticate with CHIMERA")
    print()
