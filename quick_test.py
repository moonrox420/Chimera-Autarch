...
async def test_qwen_model():
    ...
    print("\n📦 Test 1: Checking llm_integration module...")
    try:
        sys.path.insert(0, str(Path(__file__).parent))
        from llm_integration import CodeGenerator
        print("✅ llm_integration module loaded")
    except Exception as e:
        print(f"❌ Failed to load module: {e}")
        return False

    # Test 2: Initialize generator
    print("\n🔧 Test 2: Initializing Code Generator...")
    try:
        ...
        async with httpx.AsyncClient(timeout=5.0) as client:
            ...
            if response.status_code == 200:
                ...
                qwen_found = any("qwen" in m.get("name", "").lower()
                                 for m in models)
                if qwen_found:
                    print("\n✅ Qwen 2.5 Coder is ready!")
                else:
                    print("\n⚠️  Qwen 2.5 Coder not found yet")
    ...
    if qwen_found:
        print("\n🚀 Test 4: Testing code generation...")
        try:
    ...
    print("\n" + "=" * 60)
    if qwen_found:
        print("✅ ALL TESTS PASSED - Qwen 2.5 Coder is ready!")
        print("\n🚀 Next steps:")
        print("   1. Run: python test_llm.py")
        print("   2. Run: python chimera_autarch.py")
        print("   3. Visit: http://localhost:8000")
    else:
        print("⏳ Setup incomplete - waiting for model download")
        print("\n📝 To check download progress:")
        print("   Run: ollama list")
        print("\n📝 Once complete, run this test again:")
