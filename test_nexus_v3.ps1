...
function Test-Component {
  ...
  try {
    ...
    if ($LASTEXITCODE -eq 0) {
      ...
      Write-Host " âŒ FAIL" -ForegroundColor Red
...
Write-Host "Failed:        $failedTests âŒ" -ForegroundColor Red
Write-Host "Success Rate:  $successRate%" -ForegroundColor $(if ($successRate -ge 90) { "Green" } elseif ($successRate -ge 70) { "Yellow" } else { "Red" })
Write-Host ""

if ($successRate -eq 100) {
  Write-Host "ðŸŽ‰ PERFECT SCORE! All systems operational!" -ForegroundColor Green
  Write-Host ""
...
elseif ($successRate -ge 70) {
  ...
  Write-Host "âŒ CRITICAL FAILURES - Fix errors before launching" -ForegroundColor Red
